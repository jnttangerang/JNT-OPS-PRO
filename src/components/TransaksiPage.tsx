...
export default function TransaksiPage({
  ...
  const handleSaveTransaksi = async () => {
    ...
    const transactionData = {
      ...
      import_id: preInputData?.import_id || (pendingTxId?.startsWith("PRE-") ? pendingTxId : (preInputData?.transaksi_id?.startsWith("PRE-") ? preInputData.transaksi_id : "")),
      transaksi_id: (pendingTxId && !pendingTxId.startsWith("PRE-")) ? pendingTxId : ((preInputData?.transaksi_id && !preInputData.transaksi_id.startsWith("PRE-")) ? preInputData.transaksi_id : ("TRX-" + Math.floor(Date.now() / 1000) + "-" + Math.random().toString(36).substring(2, 5))),
